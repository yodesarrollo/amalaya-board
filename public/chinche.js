/* ═══════════════════════════════════════════════════════════════════════════
   LA CHINCHE · capturador de pendientes — copia de AMALAYA
   ───────────────────────────────────────────────────────────────────────────
   Copia adaptada de yodesarrollomx/yod-portal/chinche.js (plan UX v2, fase 7).
   Amalaya lleva SU PROPIA pila: base «amalayaChinche» en su propio origen
   (yodesarrollo.github.io), quien clava sale de la sesión de Amalaya, y
   «Mandar a Claude» abre el issue en yodesarrollo/amalaya-board con la
   etiqueta «chinche». Si se corrige algo del original, se trae aquí a mano.
   ───────────────────────────────────────────────────────────────────────────
...
var BD = "amalayaChinche", VER = 1, LSN = "amalaya_chinche_n";
...
/* ── quién está clavando: la sesión de Amalaya (validada por su servidor) ── */
function quien(){
  try {
    if (CTX.quien) { var q = String(CTX.quien() || "").trim(); if (q) return q; }
    var d = JSON.parse(localStorage.getItem("amalaya_sesion") || "null");
    var n = d && d.nombre ? String(d.nombre).trim() : "";
    if (n) return n;
  } catch (e) {}
  return "equipo Amalaya";
}
...
function repoActual(){ return CTX.repo || "amalaya-board"; }
function repoDeChinche(c){ return c.repo || "amalaya-board"; }
function aparato(){
...
var MAPA = {
  ...
  "amalaya|":         "src/App.jsx y src/componentes/ (Mapa3D, FichaEspacio, Financiero, Reporte, PlanAccion…)",
  "amalaya|mapa":     "src/componentes/Mapa3D.jsx y Mapa.jsx",
