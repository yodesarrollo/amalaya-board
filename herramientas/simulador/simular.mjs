...
// El mapa base sin internet: estilo mínimo de OpenFreeMap, satélite de Esri
// con mosaicos de prueba, y mosaicos vectoriales vacíos (ver mosaicos.mjs).
const ORIGEN = BASE.replace(/\/[^/]*\/$/, '')
await ctx.route(/tiles\.openfreemap\.org\/styles\//, (r) =>
  r.fulfill({ contentType: 'application/json', headers: { 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify(estilo(ORIGEN)) }))
await ctx.route(/arcgisonline\.com\/.*\/tile\/(\d+)\/(\d+)\/(\d+)/, (r) => {
  const [, z, y, x] = new URL(r.request().url()).pathname.match(/tile\/(\d+)\/(\d+)\/(\d+)/).map(Number)
  bitacora.push('mosaico de prueba: satélite')
  return r.fulfill({ contentType: 'image/png', headers: { 'Access-Control-Allow-Origin': '*' }, body: satelite(z, x, y) })
})
await ctx.route(/\/__mosaicos\//, (r) => r.fulfill({ status: 204, headers: { 'Access-Control-Allow-Origin': '*' }, body: '' }))

// Lo demás que vive en internet (fuentes, Street View) se corta rápido para
// que la página no se quede esperando; el simulador avisa en la bitácora.
await ctx.route(/fonts\.(googleapis|gstatic)\.com|maps\.google|www\.google\.com/, (r) => {
